...
selectedOrgUnit: TreeNode<OrganizationUnit>;
...
model: TreeNode<OrganizationUnit>;
action: any;
...
details(node: OrganizationUnit) {
    this.model = this.selectedOrgUnit;
    this.action = 'READ';
    this.hideCloseButton = false;
...
deleteChild(node: TreeNode<OrganizationUnit>) {
...
private expandRecursive(node: TreeNode<OrganizationUnit>, isExpand: boolean) {
...
orgToTreeNode(role: OrganizationUnit): TreeNode<OrganizationUnit> {
    ...
    label: role.name,
    data: role,
    children: role.children.map(this.orgToTreeNode)
    };
}
