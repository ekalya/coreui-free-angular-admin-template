...
export class BranchesComponent implements OnInit  {
  ...
  actionMenuClick(action: string) {
    console.log(action);
    if (action === 'CREATE') {
      this.router.navigate(['/setup/branch'], {queryParams: {branch: JSON.stringify(new Branch())}});
    } else if (action === 'EDIT') {
      this.router.navigate(['/setup/branch'], {queryParams: {branch: JSON.stringify(this.selectedBranch)}});
    }
  }
