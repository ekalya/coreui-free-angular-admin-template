...
listActionMenuClick(dynamicFormsBridge: DynamicFormsBridge) {
  ...
  if (dynamicFormsBridge.dynamicFormActions === DynamicFormActions.Create) {
    this.router.navigate(['/setup/banks/bankdetails'], {queryParams: {bank: JSON.stringify(new Bank()), mode: 'CREATE'}});
  } else if (dynamicFormsBridge.dynamicFormActions === DynamicFormActions.Update) {
    this.router.navigate(['/setup/banks/bankdetails'], {queryParams: {bank: JSON.stringify(dynamicFormsBridge.data), mode: 'EDIT'}});
  } else if (dynamicFormsBridge.dynamicFormActions === DynamicFormActions.Read) {
    this.router.navigate(['/setup/banks/bankdetails'], {queryParams: {bank: JSON.stringify(dynamicFormsBridge.data), mode: 'DETAILS'}});
  }
}
